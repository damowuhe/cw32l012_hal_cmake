...
#include <sys/time.h>
#include <sys/times.h>
#include "cw32l012_hal.h"

/* ------------------------------------------------------------------------- */
/* Default __io_putchar / __io_getchar via UART1 (polling)                   */
/* Override with your own non-weak implementation if needed.                  */
/* The user must call UART_Init() before using printf/scanf.                 */
/* ------------------------------------------------------------------------- */
...
{
    /* Wait until RX not empty, then read. ISR bit 2 = RC (Receive Complete) */
    while (CW_UART1->ISR_f.RC == 0)
    {
    }
    return (int)CW_UART1->RDR;
}
